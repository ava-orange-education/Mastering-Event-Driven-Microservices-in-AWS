...
public class CdkExampleStack extends Stack {
    ...
    private Queue createNewOrdersSQSQueue(Function newOrderServiceLambdaFunction) {
    	var queue = Queue.Builder.create(this, "SQS_NewOrderQueue")
    			.queueName("NewOrderQueue")
    			.build();
    	
		queue.grantConsumeMessages(newOrderServiceLambdaFunction);
    ...
    public CdkExampleStack(final Construct scope, final String id, final StackProps props) {
        ...
        var newOrderQueue = createNewOrdersSQSQueue(newOrderServiceLambdaFunction);
        ...
        new CfnOutput(this, "SQS_NewOrderQueue_Arn", CfnOutputProps.builder()
                .value(newOrderQueue.getQueueArn())
                .build());
    }
}
