...
private final Region region = Region.EU_CENTRAL_1;
	...
	private EventBridgeClient eventBridgeClient = EventBridgeClient.builder()
            .region(region)
            .build();
	...
	public APIGatewayV2HTTPResponse handleRequest(APIGatewayV2HTTPEvent event, Context context) {
		...
		// send OrderCreated event to EventBridge
		var entry = PutEventsRequestEntry.builder()
				.eventBusName("OrderingSystemEventBus")
	            .source("com.example.new-order-service")
	            .detailType("OrderCreated")
	            .detail(gson.toJson(order))
	            .build();
		
		var eventsRequest = PutEventsRequest.builder()
	            .entries(entry)
	            .build();
