...
private final Region region = Region.EU_CENTRAL_1;
	...
	public Void handleRequest(SQSEvent event, Context context) {
		...
		for (var message : event.getRecords()) {
			...
			// get order event
			var detail = gson.fromJson(message.getBody(), OrderEvent.class).getDetail();
			var order = gson.fromJson(gson.toJson(detail, new TypeToken<>() {}.getType()), OrderRequest.class);
