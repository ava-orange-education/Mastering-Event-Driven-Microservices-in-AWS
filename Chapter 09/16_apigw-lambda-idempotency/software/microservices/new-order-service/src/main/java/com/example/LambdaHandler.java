...
public class LambdaHandler implements RequestHandler<APIGatewayV2HTTPEvent, APIGatewayV2HTTPResponse> {
	...
	public APIGatewayV2HTTPResponse handleRequest(APIGatewayV2HTTPEvent event, Context context) {
		System.out.println("Received order request from API Gateway: " + event);

		// get the idempotency key from the request headers
		var idempotencyKey = event.getHeaders().get("x-idempotency-key");
		...
		try {
			...
			return APIGatewayV2HTTPResponse.builder()
        			...
        			.withBody("Missing X-Idempotency-Key header")
        			.build();
		}
		...
		itemValues.put("idempotencyKey",  AttributeValue.builder().s(idempotencyKey).build());
		...
		try {
			...
			return APIGatewayV2HTTPResponse.builder()
        			.withStatusCode(HttpStatusCode.OK)
        			.withBody("Request already processed")
        			.build();
		} catch(Exception e) {
